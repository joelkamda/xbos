...
router = APIRouter(tags=["XafPay Webhooks"])
...
async def xafpay_webhook(
    ...
    """
    XafPay Gateway webhook.

    Responsibilities:
    - Verify signature
    - Resolve PaymentIntent
    - Construct WebhookEvent
    - Delegate settlement to PaymentService
    """
    ...
    try:
        ...
        # -------------------------------------------------
        # Resolve intent globally (multi-tenant safe)
        # -------------------------------------------------
        ...
        if not intent:
            raise ValueError(
                f"PaymentIntent not found for gateway_intent_id={gateway_intent_id}"
            )

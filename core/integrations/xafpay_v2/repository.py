...
class XafPayV2Repository:
    ...
    @classmethod
    def attempt_by_gateway_payment_id(
        cls, session, payment_id: str, *, lock: bool = False
    ) -> AttemptAuthority | None:
        suffix = "FOR UPDATE" if lock else ""
        public_id = session.execute(text(f"""
            SELECT public_id FROM canonical_payment_attempts
             WHERE orchestrator_code='xafpay'
               AND external_attempt_reference=:payment_id
             ORDER BY id DESC LIMIT 1
             {suffix}
        """), {"payment_id": payment_id}).scalar_one_or_none()
        return cls.attempt_authority(
            session, UUID(str(public_id)), lock=lock
        ) if public_id else None

    @classmethod
    def bind_checkout_session(
        cls,
        session,
        ...
        *,
        ...
        existing = metadata.get("checkout_session_id")
        if existing and existing != checkout_session_id:
            raise RuntimeError("XAFPAY_CHECKOUT_SESSION_IDENTITY_CONFLICT")
        if existing == checkout_session_id and metadata.get("checkout_status") == checkout_status:
            return attempt
        metadata["checkout_session_id"] = checkout_session_id
        metadata["checkout_status"] = checkout_status
        ...
        if result.rowcount != 1:
            raise RuntimeError("XAFPAY_CHECKOUT_BINDING_VERSION_CONFLICT")
        updated = cls.attempt_authority(session, attempt.public_id, lock=True)
        if updated is None:
            raise RuntimeError("XAFPAY_CHECKOUT_BINDING_MISSING")
        return updated

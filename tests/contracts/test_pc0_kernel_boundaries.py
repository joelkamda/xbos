...
class PC0KernelBoundaryTests(unittest.TestCase):
    ...
    @staticmethod
    def _finance_contracts() -> tuple[dict, dict]:
    ...
    @classmethod
    def _copy_frozen_finance(cls, destination: Path) -> tuple[dict, dict]:
        baseline, inventory = cls._finance_contracts()
    ...
    def test_frozen_finance_explicit_inventory_passes(self) -> None:
        baseline, inventory = self._finance_contracts()
        _validate_finance(ROOT, baseline, inventory)

    def test_finance_runtime_cache_artifacts_do_not_change_fingerprint(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            candidate_root = Path(directory)
            baseline, inventory = self._copy_frozen_finance(candidate_root)
            ...
            _validate_finance(candidate_root, baseline, inventory)

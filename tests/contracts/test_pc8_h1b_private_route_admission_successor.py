...
def test_pc8_is_posthoc_successor_only_without_new_schema_head():
    ...
    nce = contract["nce_h1b_additive_successor"]
    assert nce["authority"] == "XBOS-NCE-H1B-ADDITIVE-SUCCESSOR-R1-SOURCE-MATERIALIZATION"
    assert nce["prior_h1b_candidate_path_count"] == 39
    assert nce["nce_unique_addition_count"] == 7
    assert nce["current_combined_candidate_path_count"] == 46
    assert len(nce["nce_unique_additions"]) == 7
    assert nce["overlap_count"] == 3
    assert nce["historical_manifest_rewrites"] == 0
...
def test_private_prefix_bypasses_human_tenant_and_branch_admission_only():
    ...
    overlap = contract["nce_h1b_additive_successor"]["overlap_sha256"]
    for relative in (
        "core/middleware/auth_middleware.py",
        "core/middleware/tenant_middleware.py",
        "core/middleware/branch_middleware.py",
        ...
        assert text.count(private_needle) == 1
        assert text.count(nce_needle) == 1
        current = hashlib.sha256((ROOT / relative).read_bytes().replace(b"\r\n", b"\n")).hexdigest()
        assert current == overlap[relative]
...
def test_pc8_successor_verifier_proves_all_required_boundaries():
    ...
    assert result["h1b_current_candidate_path_count"] == 39
    assert result["nce_unique_addition_count"] == 7
    assert result["overlap_count"] == 3
    assert result["current_candidate_path_count"] == 46

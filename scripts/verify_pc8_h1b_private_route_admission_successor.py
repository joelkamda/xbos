...
def _sha_without_private_bypass(relative: str) -> str:
    path = ROOT / relative
    text = path.read_text(encoding="utf-8")
    needle = f'            or path.startswith("{PRIVATE_PREFIX}")\n'
    if text.count(needle) != 1:
        raise RuntimeError(f"private-prefix bypass count mismatch={relative}")
...
def verify() -> dict:
    ...
    for relative in (
        ...
        if current.count(private_line) != 1:
            raise RuntimeError(f"private-prefix admission missing or duplicated={relative}")
        if _sha_without_private_bypass(relative) != successor["accepted_base_canonical_sha256"][relative]:
            raise RuntimeError(f"non-private middleware behavior changed={relative}")
    ...
    if changed != expected_current:
        raise RuntimeError(
            f"candidate-path succession mismatch missing={sorted(expected_current-changed)} "
            f"extra={sorted(changed-expected_current)}"
        )
    ...
    return {
        ...
        "current_candidate_path_count": 39,
